...
%   ac(:,i) = a(:,i)+cross(dw(:,i),pc(:,i-1))+cross(w(:,i),cross(wa(:,i),pc(:,i-1)));
    ...
    dw(:,i) = R(:,:,i-1)'*(dw(:,i-1)+ddq(i-1)*[0 0 1]'+cross(v(i-1)*w(:,i-1),[0 0 1]'));
    a(:,i) = R(:,:,i-1)'*a(:,i-1)+cross(dw(:,i),p(:,i-1))+cross(wa(:,i),cross(w(:,i),p(:,i-1)));
    ac(:,i) = a(:,i)+cross(dw(:,i),pc(:,i-1))+cross(wa(:,i),cross(w(:,i),pc(:,i-1)));
end
