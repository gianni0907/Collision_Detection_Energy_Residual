function plot_data_redobs(q_des,dq_des,q,dq,dq_hat,u,u_ext,r,P_ext,s,vbar,eta,t)
...
plot(t,dq(1,:)-dq_hat(1,:));
plot(t,dq(2,:)-dq_hat(2,:));
plot(t,dq(3,:)-dq_hat(3,:));
...
% plot the switching gain scheduling result
norm_dq = zeros(size(t));
norm_dqhat = zeros(size(t));
for i=1:size(t,2)
    norm_dq(i) = norm(dq(:,i));
    norm_dqhat(i) = norm(dq_hat(:,i));
end
figure();
hold on,grid on,xlabel("$[s]$","Interpreter","Latex"),ylabel("$[rad/s]$","Interpreter","Latex"),title("Gain scheduling");
plot(t,norm_dq);
plot(t,norm_dqhat+eta);
stairs(linspace(0,10,size(s,2)),s*vbar,'--');
legend('$|\dot{q}|$','$|\hat{\dot{q}}|+\eta$','$s\bar{v}$',"Interpreter","Latex");
end